...
.lh-150 { line-height: 1.5; }
    ...
    <%for(int i = 0 ; i<14; i++){%>
       ...
       <div class="my-3 p-3 bg-white rounded box-shadow">
        <h6 class="border-bottom border-gray pb-2 mb-0"><%= c.get(Calendar.DAY_OF_MONTH)%> число <%=c.get(Calendar.MONTH)%> месяца <%= c.get(Calendar.YEAR)%> года</h6>
...
%>#2A357C;<%
       ...
       <input type="hidden" name="hour" value="<%=days.get(j).getTime()%>">
        ...
        <input type="hidden" name="month" value="<%=days.get(j).getMonth()%>">
       <input type="hidden" name="hour" value="<%=days.get(j).getTime()%>">
        <button class="btn btn-sm  btn-outline-primary" type="submit">Удалить час работы</button>
        </form> 
            </div>
            <span class="d-block">Время: <%=days.get(j).getTime()%><%=days.get(j).getDate()%>-е число </span>
   ...
   %>
    ...
    <small class="d-block text-right mt-3 d-flex">
        ...
        <button class="btn btn-sm  btn-outline-primary" type="submit">Добавить время на <%= c.get(Calendar.DAY_OF_MONTH)%> <%=c.get(Calendar.MONTH)%> <%= c.get(Calendar.YEAR)%></button>
        </form>  
     ...
     <%if(canDeleteDay){%>
        ...
        <input type="hidden" name="month" value="<%=c.get(Calendar.MONTH)%>">
        <input type="hidden" name="year" value="<%= c.get(Calendar.YEAR)%>">
        <button class="btn btn-sm  btn-outline-primary"  type="submit">Удалить <%= c.get(Calendar.DAY_OF_MONTH)%> <%=c.get(Calendar.MONTH)%> <%= c.get(Calendar.YEAR)%> с рабочего графика</button>
        </form>
         <%}%>
